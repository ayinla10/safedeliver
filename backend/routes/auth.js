...
router.post('/register', authLimiter, async (req, res) => {
    try {
        const schema = z.object({ full_name: z.string().min(2), email: z.string().email(), phone: z.string().min(10), password: z.string().min(8) });
...
router.post('/verify-otp', async (req, res) => {
    try {
        ...
        const result = await db.query('SELECT * FROM sellers WHERE phone = $1', [phone]);
...
router.post('/login', authLimiter, async (req, res) => {
    try {
        ...
        const result = await db.query('SELECT * FROM sellers WHERE phone = $1 OR email = $1', [phone]);
        if (result.rows.length === 0) return res.status(401).json({ error: 'Invalid credentials' });
        const seller = result.rows[0];
        if (seller.locked_until && new Date(seller.locked_until) > new Date()) {
            return res.status(423).json({ error: 'Account locked. Try again later.' });
        }
        if (!seller.is_verified) return res.status(403).json({ error: 'Account not verified. Please verify OTP.' });
